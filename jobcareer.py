...
browser.implicitly_wait(3)
time.sleep(2)
browser.find_element_by_xpath('//*[@id="pagingBar"]/a[4]').click()
...
for i in range(0, last_page_num):
    ...
    try:
        ...
        browser.implicitly_wait(3)
        time.sleep(2)
        url_list = [element_list[j].find_element_by_css_selector('td.title > a').get_attribute("href") for j in
        ...
        titles = [element_list[j].find_element_by_css_selector('td.title').text for j in
                  range(len(element_list) - 1, notice_len - 1, -1)]
